...
- **`index.html`** — the landing page. The trainer, the consolidated Play page, and
...
- **`play.html`** — **Play**: a *consolidated* game that adapts to `settings.players`
  (the global "Players" setting). Currently supports **2 (heads-up)**, **3**, **4** and
  **5** (cutthroat); the reducer is general (deal/crib/show/pegging/layout all computed
  from the player count via `plan(P, dealerIdx)`), so adding 6 is mostly widening `PLAYER_OPTIONS`.
