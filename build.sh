...
# The PRIMARY-language i18n head, INLINED into each page: the runtime (i18n.js) + language list
# (locales/index.js) + English catalogue (locales/en.js). Inlining the default path means it
# needs zero external fetch — no flash, and it can't be broken by a host/deploy that fails to
# serve those files (e.g. the Pages workflow that once omitted them). NON-English locales are
# NOT inlined: i18nBootstrap() lazy-loads only the picked one via <script src="locales/<code>.js">.
# (No locale string contains "</script>", so inlining the catalogue can't break out of the block.)
i18n_head() {
