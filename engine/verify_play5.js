#!/usr/bin/env node
/* Verifies the 5-handed game (play5.html) without a browser — same harness as
 * verify_play.js, adapted for five seats and the 5-handed deal.
 *
 * 5-handed rule: the dealer is dealt FOUR cards and throws none; every other player
 * is dealt five and throws one. So the crib is the four non-dealer throws (no deck
 * card), and when the human deals they skip the discard phase entirely.
 *
 * We eval the compiled play5.html <script> in a vm sandbox and drive whole hands,
 * asserting:
 *   - the dealer is dealt 4 and makes no throw; the crib = the four non-dealer throws
 *   - the human-dealer case skips discard and goes straight to the cut
 *   - go / 31 / last-card never double-count; turn rotates over five seats
 *   - his heels = +2 at the cut; the show counts five seats + crib with the 121 stop
...
const html = fs.readFileSync(path.join(__dirname, "..", "play5.html"), "utf8");
...
function playHand(state) {
  ...
  check(state.seats[d].dealt.length === 4, "dealer is dealt four");
  for (let i = 0; i < 5; i++) if (i !== d) check(state.seats[i].dealt.length === 5, `seat ${i} dealt five`);
  for (let i = 1; i < 5; i++) {
    if (i === d) check(state.seats[i].kept.length === 4 && !state.seats[i].discard, `dealer seat ${i} keeps 4, throws none`);
    else check(state.seats[i].kept.length === 4 && state.seats[i].discard, `non-dealer seat ${i} bot throw computed`);
  }
  if (d === 0) {
    check(state.phase === "cut", "human deals -> straight to cut (no throw)");
    check(state.seats[0].kept.length === 4 && !state.seats[0].discard, "human dealer keeps 4, throws none");
  } else {
  ...
  // the crib is exactly the four non-dealer throws (no deck card)
  const throws = state.seats.map((s, i) => (i === d ? null : s.discard)).filter(Boolean);
  check(throws.length === 4, "four non-dealer throws exist");
  for (const t of throws) check(state.crib.some((c) => sameCard(c, t)), "each non-dealer throw is in the crib");
  ...
  check(state.peg.hands.length === 5, "five pegging hands");
  check(state.peg.hands.every((h) => h.length === 4), "everyone pegs from four cards");

  let guard = 0;
  while (state.phase === "play" && guard++ < 500) {
...
/* ---- B. drive many hands; scores only go up, stay sane ---- */
{
  ...
  let hands = 0, exceptions = 0, humanDealt = 0;
  for (let h = 0; h < 90 && state.phase !== "over"; h++) {
    try {
      if (state.dealerIdx === 0) humanDealt++;
  ...
  check(humanDealt >= 1, "exercised the human-as-dealer (skip-discard) path at least once");
  check(state.phase === "over" || hands === 90, "reached a winner or ran the cap");
}

/* ---- C. his heels = +2 at the cut (starter is deck[24] for 5-handed) ---- */
{
  ...
  state = { ...state, deck: state.deck.map((c, i) => (i === 24 ? { r: 11, s: 0 } : c)) };
  state = reduce(state, { type: "CUT" });
...
/* ---- D. show 121 short-circuit: pone (counted first) pegs out before others ---- */
{
  ...
  let fab = {
    ...
    phase: "show", show: { order: [pone, second, (d + 3) % 5, (d + 4) % 5, d, "CRIB"], step: 0, scored: false, claimSubmitted: false, claimValue: null },
...
/* ---- E. muggins: under-claim hands missed points to the next opponent ---- */
{
  ...
  const human = 0;
  const order = [human, (human + 1) % 5, (human + 2) % 5, (human + 3) % 5, (human + 4) % 5, "CRIB"];
...
/* ---- F. cut for deal: five draw; lowest unique deals; game starts in cutdeal ---- */
{
  const { drawForDealer } = S;
  for (let i = 0; i < 80; i++) {
    const { dealerIdx, draw } = drawForDealer();
    check(draw.length === 5, "five players draw for deal");
  ...
  const g = initGame();
  check(g.phase === "cutdeal" && Array.isArray(g.dealDraw) && g.dealDraw.length === 5, "new game starts in cutdeal with a 5-card draw");
}

console.log(`\nplay5.html engine checks: ${ok} passed, ${fail} failed`);
