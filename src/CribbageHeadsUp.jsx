...
/* ============================================================
   VERIFIED CRIBBAGE ENGINE — copied verbatim from CribbagePlay.jsx /
   CribbageTrainer.jsx. The pages are independent self-contained HTML files and do
   not share a module, matching how the repo already works (the engine/ scripts also
...
/* ===== Pegging ===== suits are irrelevant; pile/hand handed to pegScore/pegChoose
   are ranks 1..13. Mechanics unit-tested in engine/pegging.js. */
...
/* Per-rank crib value ("crib swing"), the "your" row from CLAUDE.md. */
const CRIB_VALUE = [3.96, 3.95, 4.05, 4.06, 6.38, 4.10, 4.21, 4.34, 4.09, 3.74, 4.19, 3.73, 3.85];

// Heads-up you throw TWO cards into the crib. Estimate their combined value as a
// crib seed: ~one-card-equivalent of each, plus bonuses for a pair / a fifteen /
// run adjacency the two cards make together. A fast stand-in for Monte-Carlo cribDetail.
function cribSeed(a, b) {
  let v = (CRIB_VALUE[a.r - 1] + CRIB_VALUE[b.r - 1]) * 0.5;
  if (a.r === b.r) v += 2;                                       // a pair in the crib
  else if (Math.abs(a.r - b.r) <= 2) v += 0.5;                   // run potential
  if (fifteenVal(a.r) + fifteenVal(b.r) === 15) v += 2;          // a fifteen
  return v;
}
...
function aiDiscard(dealt6, seat, dealerIdx) {
  ...
  for (const idxs of twoCombos(6)) {
    const four = dealt6.filter((_, j) => !idxs.includes(j));
    const thrown = idxs.map((i) => dealt6[i]);
    const val = handDetail(four, dealt6).ev + sign * cribSeed(thrown[0], thrown[1]);
    if (val > bestVal) { bestVal = val; best = { discard: thrown, kept: four }; }
  }
  return best;
}
...
const initPeg = (seats, dealerIdx) => ({
  ...
  turn: (dealerIdx + 1) % 2,        // pone (non-dealer) leads
...
function evalDiscards(dealt6, dealerIdx) {
  ...
  const opts = twoCombos(6).map((idxs) => {
    const four = dealt6.filter((_, j) => !idxs.includes(j));
    const thrown = idxs.map((i) => dealt6[i]);
    const hd = handDetail(four, dealt6);
    const cribSwing = cribSeed(thrown[0], thrown[1]);
...
// Commit the human's two-card throw: assemble the 4-card crib (your 2 + opponent's 2).
function commitDiscard(state, idxs) {
  ...
  return { ...state, seats, crib: seats.flatMap((s) => s.discard), pendingDiscard: null, phase: "cut" };
}

function dealNewHand(state) {
  const deck = freshDeck();
  const seats = [0, 1].map((i) => ({
    score: state.seats[i].score, isAI: i === 1, history: state.seats[i].history || [],
    dealt: sortHand(deck.slice(i * 6, i * 6 + 6)), kept: null, discard: null,
  }));
  const ai = aiDiscard(seats[1].dealt, 1, state.dealerIdx);
  seats[1].discard = ai.discard; seats[1].kept = sortHand(ai.kept);
  return {
...
// Commit one pegging card for `seat`. Mirrors the verified playPegging (P=2).
...
function reduce(state, action) {
  switch (action.type) {
    ...
    case "CUT": {
      const starter = state.deck[12]; // the next undealt card after 2 hands of 6
    ...
    case "PASS_GO": {
      ...
      if (passes >= 2) { // both players unable to play -> award the go
      ...
      return { ...state, peg: { ...peg, passes, turn: (seat + 1) % 2 }, message: `${sv(seat, "say", "says")} "go".` };
    }
    ...
    case "SHOW_NEXT": {
      ...
      if (humanCount) {
        ...
        if (missed > 0 && winner === null) {
          seats = addScore(seats, 1, missed, "muggins"); // the opponent takes what you miss
          message = `Muggins! ${seatName(1)} claims the ${missed} you missed (had ${info.total}).`;
          if (seats[1].score >= TARGET) winner = 1;
        } else {
...
function drawForDealer() {
  for (let attempt = 0; attempt < 500; attempt++) {
    const draw = freshDeck().slice(0, 2);
...
function newGameState(prev) {
  ...
  return {
    seats: [0, 1].map((i) => ({ score: 0, isAI: i === 1, dealt: [], kept: null, discard: null, history: [] })),
...
function ScoreRow({ seats, dealerIdx, turn, winner, onPick }) {
  return (
    <div style={{ display: "grid", gridTemplateColumns: "repeat(2,1fr)", gap: 6, margin: "0 0 6px" }}>
      {seats.map((s, i) => {
        const isTurn = turn === i;
...
// A seat's cards: remaining cards face-down on the left, played cards fanned on top.
  ...
  return (
    <div style={{ marginTop: 12, display: "flex", flexDirection: "column", gap: 14 }}>
      ...
      <div style={{ display: "flex", justifyContent: "center" }}>
        <SeatCell i={1} dealerIdx={dealerIdx} active={peg.turn === 1} played={peg.played[1]} remaining={peg.hands[1].length} />
      </div>
      ...
      {/* your played cards, just above the pile */}
      ...
      {needClaim ? (
        <div style={{ background: "rgba(0,0,0,0.26)", borderRadius: 10, padding: "14px 14px 16px" }}>
          <div style={{ fontSize: 13.5, lineHeight: 1.5, marginBottom: 12 }}>
            ...
            miss any and your opponent takes them.
          </div>
...
function DiscardWarning({ pd, dealer, dispatch }) {
  ...
  const side = dealer ? "for you" : "to the opponent";
  const Line = ({ label, o, strong }) => (
    <div style={{ fontFamily: mono, fontSize: 11.5, lineHeight: 1.6, color: strong ? T.cream : T.muted }}>
      <b style={{ color: strong ? T.good : T.ivory }}>{label}</b> throw {o.thrown.map(tag).join(" ")} · keep {o.four.map(tag).join(" ")} · hand {o.keptEV.toFixed(2)} · crib {o.cribSwing.toFixed(2)} {side} → net <b>{o.value.toFixed(2)}</b>
    </div>
  );
  return (
    <Panel tone="red">
      ...
      <div style={{ fontSize: 12.5, lineHeight: 1.5, color: T.cream, marginBottom: 12 }}>
        “hand” = your kept four averaged over every cut; “crib” = the value of the two thrown cards in the crib
        ({dealer ? "added to your score" : "given to the opponent, so subtracted"}). Net is the two combined.
      </div>
      <div style={{ display: "flex", gap: 8 }}>
        ...
        }}>Throw {chosen.thrown.map(tag).join("+")} anyway</button>
  ...
  return (
    <div style={{ background: "rgba(0,0,0,0.32)", border: `1px solid ${T.line}`, borderRadius: 12, padding: "14px 16px 4px", marginBottom: 14 }}>
      ...
      <Row title="Counting" k="counting"
        desc="Auto tallies every hand for you. Muggins: you claim your own hand (and crib when you deal) — miss points and your opponent takes them."
...
/* ============================ APP ============================ */
export default function CribbageHeadsUp() {
  ...
  return (
    <div style={{
      ...
      <style>{`
        ...
        .dealwrap > *:nth-child(2){animation-delay:45ms}
        .dealwrap > *:nth-child(3){animation-delay:90ms}
        .dealwrap > *:nth-child(4){animation-delay:135ms}
        .dealwrap > *:nth-child(5){animation-delay:180ms}
        .dealwrap > *:nth-child(6){animation-delay:225ms}
      ...
      <header style={{
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 12 }}>
          <div>
            <h1 style={{ margin: 0, fontSize: 22, color: "#2A1B0E", letterSpacing: 0.3, fontWeight: 700 }}>Cribbage — Heads-Up</h1>
            <span style={{ display: "block", fontFamily: mono, fontSize: 11, color: "rgba(42,27,14,0.75)", marginTop: 2 }}>2-handed vs the computer · first to 121</span>
          </div>
        ...
        {phase === "deal" && (
          <div style={{ marginTop: 14, display: "flex", flexDirection: "column", gap: 16 }}>
            <Panel tone={dealer ? "good" : null}>
              ...
              <div style={{ fontFamily: mono, fontSize: 11.5, color: T.muted, marginTop: 3 }}>Each player gets 6 cards and throws two to the crib. First to 121 wins.</div>
            </Panel>
        ...
        {phase === "discard" && (
          <div style={{ marginTop: 14, display: "flex", flexDirection: "column", gap: 14 }}>
            <Panel tone={dealer ? "good" : "red"}>
              <div style={{ fontWeight: 700, fontSize: 15 }}>{dealer ? "Your crib — be greedy" : "Feeds the opponent's crib — defend"}</div>
              <div style={{ fontFamily: mono, fontSize: 11.5, color: T.muted, marginTop: 3 }}>
                Tap the two cards you'll throw to the crib.{sel.length === 1 ? " One more…" : ""}
              </div>
            </Panel>
            <div style={{ display: "flex", justifyContent: "center" }}>
              <SeatCell i={1} dealerIdx={dealerIdx} remaining={6} />
            </div>
