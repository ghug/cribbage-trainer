...
/* ============================================================
   PASS & PLAY — two-player hot-seat cribbage on one shared device. Both seats are
   human; a full-screen "pass the device" privacy screen hides one player's hand from
   the other between turns, and every move is select-then-confirm. Self-contained: the
   verified engine primitives are copied verbatim (the pages never share a module). No
   bots, no game history (yet). First to 121.
   ============================================================ */
...
function Card({ card, onClick, clickable, dim, raised, small }) {
  const [hover, setHover] = React.useState(false);
  const base = small ? 44 : 68;
  const lift = hover && clickable ? -6 : 0;
  const edge = raised ? T.selBlue : null;
  return (
    <div style={{ display: "flex", flexDirection: "column", alignItems: "center", flex: `0 1 ${base}px`, minWidth: 0, maxWidth: base }}>
      <button
        ...
        style={{
          ...
          boxShadow: raised ? "0 8px 18px rgba(0,0,0,0.45)" : "0 4px 10px rgba(0,0,0,0.35)",
...
function ConfirmButton({ label, enabled, onClick }) {
  return (
    <button onClick={enabled ? onClick : undefined} disabled={!enabled} style={{
      width: "100%", padding: "12px", borderRadius: 10, border: "none", cursor: enabled ? "pointer" : "default",
      ...
      fontSize: 15, fontWeight: 700, letterSpacing: 0.3, boxShadow: enabled ? "0 4px 12px rgba(0,0,0,0.35)" : "none",
    }}>{label}</button>
  );
}
...
// Deal a fresh hand with the given dealer: 6 cards each, the rest left for the starter
// cut. The non-dealer (pone) discards/plays first.
function dealState(dealer) {
  const deck = freshDeck();
  return {
    dealer,
    hands: [sortHand(deck.slice(0, 6)), sortHand(deck.slice(6, 12))],
    deck: deck.slice(12),
    crib: [], discards: [null, null], starter: null, peg: null, show: null,
    turn: 1 - dealer, holder: null, phase: "discard", message: "",
  };
}
function initGame() {
  const dealer = Math.random() < 0.5 ? 0 : 1;
  return { dealer, scores: [0, 0], winner: null, phase: "start", hands: [[], []], deck: [], crib: [], discards: [null, null], starter: null, peg: null, show: null, turn: 1 - dealer, holder: null, message: "" };
}
...
function playCard(state, card) {
  ...
  const scores = state.scores.slice(); scores[seat] += pts;
  let message = pts > 0 ? `${NAMES[seat]}: ${scoreCallout(pile, count, pts)} (count ${count}).` : `${NAMES[seat]} plays ${tag(card)} (count ${count}).`;
  const np = { ...peg, count, pile, pileSuited, played, lastPlayer: seat, passes: 0 };
  if (count === 31) { np.count = 0; np.pile = []; np.pileSuited = []; np.lastPlayer = -1; }
  if (scores[seat] >= TARGET) return { ...state, hands, peg: np, scores, message, phase: "over", winner: seat };
  const remaining = hands[0].length + hands[1].length;
  if (remaining === 0) {
    if (np.lastPlayer >= 0) {
      scores[seat] += 1; message += ` +1 for last card.`;
      if (scores[seat] >= TARGET) return { ...state, hands, peg: np, scores, message, phase: "over", winner: seat };
    }
    return enterShow({ ...state, hands, peg: np, scores, message });
  }
  return { ...state, hands, peg: np, scores, message, turn: 1 - seat };
}

function sayGo(state) {
  const peg = state.peg, seat = state.turn;
  const passes = peg.passes + 1;
  const scores = state.scores.slice();
  let message = `${NAMES[seat]} says "go".`;
  if (passes >= 2) {
    const np = { ...peg, passes: 0 };
    if (peg.lastPlayer >= 0 && peg.count !== 31) {
      scores[peg.lastPlayer] += 1; message = `${NAMES[peg.lastPlayer]} +1 for the go.`;
      if (scores[peg.lastPlayer] >= TARGET) return { ...state, peg: np, scores, message, phase: "over", winner: peg.lastPlayer };
    }
    np.count = 0; np.pile = []; np.pileSuited = []; np.lastPlayer = -1;
    return { ...state, peg: np, scores, message, turn: 1 - seat };
  }
  return { ...state, peg: { ...peg, passes }, message, turn: 1 - seat };
}

// Build the three show items (pone hand, dealer hand, dealer's crib) and award the
// first (pone counts first and can peg out before the dealer counts).
function enterShow(state) {
  const pone = 1 - state.dealer;
  const mk = (owner, four, isCrib) => { const acc = [0, 0, 0, 0, 0]; const pts = scoreInto(four, state.starter, isCrib, acc); return { owner, four, isCrib, pts, acc }; };
  const items = [mk(pone, state.peg.played[pone], false), mk(state.dealer, state.peg.played[state.dealer], false), mk(state.dealer, state.crib, true)];
  const scores = state.scores.slice();
  scores[items[0].owner] += items[0].pts;
  const won = scores[items[0].owner] >= TARGET;
  return { ...state, phase: won ? "over" : "show", winner: won ? items[0].owner : null, scores, show: { items, step: 0 }, message: "" };
}

function showNext(state) {
  const { items, step } = state.show;
  if (step >= items.length - 1) {
    const nd = 1 - state.dealer;
    return { ...state, ...dealState(nd), scores: state.scores, winner: null };
  }
  const next = step + 1;
  const scores = state.scores.slice();
  scores[items[next].owner] += items[next].pts;
  const won = scores[items[next].owner] >= TARGET;
  return { ...state, scores, show: { items, step: next }, phase: won ? "over" : "show", winner: won ? items[next].owner : null };
}

function reduce(state, a) {
  switch (a.type) {
    case "NEW_GAME": return initGame();
    case "DEAL": return { ...state, ...dealState(state.dealer), scores: state.scores, winner: null };
    case "READY": return { ...state, holder: state.turn };
    case "DISCARD": {
      const seat = state.turn, hand = state.hands[seat];
      const thrown = a.idxs.map((i) => hand[i]);
      const hands = state.hands.slice(); hands[seat] = sortHand(hand.filter((_, j) => !a.idxs.includes(j)));
      const discards = state.discards.slice(); discards[seat] = thrown;
      const other = 1 - seat;
      if (discards[other] == null) return { ...state, hands, discards, turn: other };
      const crib = [...discards[0], ...discards[1]];
      return { ...state, hands, discards, crib, phase: "cut", turn: 1 - state.dealer };
    }
    case "CUT": {
      const starter = state.deck[0];
      const scores = state.scores.slice();
      let message = `Cut: ${tag(starter)}.`, winner = null, phase = "play";
      if (starter.r === 11) {
        scores[state.dealer] += 2; message += ` His heels — ${NAMES[state.dealer]} pegs 2.`;
        if (scores[state.dealer] >= TARGET) { winner = state.dealer; phase = "over"; }
      }
      const peg = { count: 0, pile: [], pileSuited: [], played: [[], []], passes: 0, lastPlayer: -1 };
      return { ...state, starter, scores, message, phase, winner, peg, turn: 1 - state.dealer, holder: null };
    }
    case "PLAY": return playCard(state, a.card);
    case "GO": return sayGo(state);
    case "SHOW_NEXT": return showNext(state);
    default: return state;
  }
}

/* ============================ UI ============================ */
function Scoreboard({ scores, dealer, turn, winner }) {
  return (
    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8, marginBottom: 10 }}>
      {[0, 1].map((i) => {
        const isTurn = turn === i && winner === null;
        const isWin = winner === i;
        return (
          <div key={i} style={{
            padding: "8px 8px 9px", borderRadius: 9, textAlign: "center", minWidth: 0,
            ...
            <div style={{ fontFamily: mono, fontSize: 11, color: T.muted, whiteSpace: "nowrap" }}>{NAMES[i]}{dealer === i ? <span style={{ color: T.pegIvory, fontWeight: 700 }} title="dealer — has the crib"> ⬤D</span> : ""}</div>
            <div style={{ fontFamily: serif, fontWeight: 700, fontSize: 24, color: isWin ? T.good : T.ivory }}>{scores[i]}</div>
            <div style={{ marginTop: 6, padding: "0 2px" }}><PegTrack pct={(scores[i] / TARGET) * 100} /></div>
          </div>
        );
      })}
    </div>
...
function CardRow({ cards, small }) {
  return (
    <div style={{ display: "flex", gap: 6, justifyContent: "center", flexWrap: "wrap" }}>
      {cards.map((c) => <Card key={cardId(c)} card={c} small={small} />)}
    </div>
  );
}
...
export default function CribbagePassPlay() {
  ...
  const { phase, dealer, hands, crib, starter, scores, turn, holder, peg, show, message, winner } = state;
  const pone = 1 - dealer;

  useEffect(() => { setSel([]); setSelCard(null); }, [turn, phase, holder]);
  // A player with no cards left can't act — auto-pass for them so the other plays on.
  useEffect(() => {
    if (phase === "play" && winner === null && hands[turn] && hands[turn].length === 0) {
      const t = setTimeout(() => dispatch({ type: "GO" }), 200);
  ...
  const skunkNote = () => {
    ...
    const loser = 1 - winner, ls = scores[loser];
    if (ls <= 60) return " Double skunk 🦨🦨!";
    if (ls <= 90) return " Skunk 🦨!";
    return null;
  };
  ...
  return (
    <div style={{ minHeight: "100%", background: `radial-gradient(120% 90% at 50% 0%, ${T.baizeHi}, ${T.baize})`, color: T.cream, fontFamily: serif }}>
      ...
      <main style={{ maxWidth: 560, margin: "0 auto", padding: "16px 16px 28px" }}>
        {phase !== "start" && <Scoreboard scores={scores} dealer={dealer} turn={turn} winner={winner} />}
        {message && phase !== "start" && <div style={{ fontFamily: mono, fontSize: 11.5, color: T.muted, textAlign: "center", margin: "0 0 12px" }}>{message}</div>}

        {phase === "start" && (
          <div style={{ display: "flex", flexDirection: "column", gap: 16, marginTop: 10 }}>
            <Panel>
              <div style={{ fontWeight: 700, fontSize: 16 }}>Pass &amp; Play</div>
              <div style={{ fontFamily: mono, fontSize: 11.5, color: T.muted, marginTop: 4, lineHeight: 1.6 }}>
                Two players share one device. <b style={{ color: T.cream }}>{NAMES[dealer]}</b> deals first. After each move a pass screen hides your hand — hand the device to the other player and they tap to reveal theirs.
              </div>
            </Panel>
            {bigBtn("Deal", () => dispatch({ type: "DEAL" }), "wood")}
          </div>
        )}
        ...
        {phase === "cut" && (
          <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
            <Panel>
              ...
              <div style={{ fontFamily: mono, fontSize: 11.5, color: T.muted, marginTop: 3 }}>Four cards in {NAMES[dealer]}'s crib. {NAMES[pone]} cuts for the starter.</div>
            </Panel>
            <div style={{ display: "flex", justifyContent: "center", gap: 8 }}>{[0, 1, 2, 3].map((i) => <CardBack key={i} />)}</div>
            {bigBtn("Cut the deck", () => dispatch({ type: "CUT" }), "wood")}
          </div>
        )}
        ...
        {phase === "show" && show && (
          <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
            {show.items.slice(0, show.step + 1).map((it, k) => (
              <Panel key={k} tone={k === show.step ? "good" : null}>
                <div style={{ fontWeight: 700, fontSize: 14, marginBottom: 8 }}>{NAMES[it.owner]} — {it.isCrib ? "crib" : "hand"} scores {it.pts}</div>
                <CardRow cards={[...it.four, starter]} small />
                <div style={{ fontFamily: mono, fontSize: 11, color: T.muted, marginTop: 8, textAlign: "center" }}>with the {tag(starter)} starter · {showLabel(it.acc)}</div>
              </Panel>
            ))}
            {bigBtn(show.step >= show.items.length - 1 ? `Next hand (${NAMES[1 - dealer]} deals)` : "Next", () => dispatch({ type: "SHOW_NEXT" }), "wood")}
          </div>
        )}
        ...
        {phase === "over" && (
          <div style={{ display: "flex", flexDirection: "column", gap: 16, marginTop: 4 }}>
            <Panel tone="good">
              <div style={{ fontWeight: 700, fontSize: 18 }}>{NAMES[winner]} wins! 🎉</div>
              <div style={{ fontFamily: mono, fontSize: 11.5, color: T.muted, marginTop: 3 }}>Final: {NAMES[0]} {scores[0]} · {NAMES[1]} {scores[1]}.{skunkNote()}</div>
            </Panel>
            {bigBtn("New game", () => dispatch({ type: "NEW_GAME" }), "wood")}
          </div>
        )}
      </main>
      ...
      {confirmHome && (
        <div style={{ position: "fixed", inset: 0, zIndex: 220, background: "rgba(0,0,0,0.62)", display: "flex", alignItems: "center", justifyContent: "center", padding: 20 }} onClick={() => setConfirmHome(false)}>
          <div onClick={(e) => e.stopPropagation()} style={{ maxWidth: 340, width: "100%", background: T.baize, border: `1px solid ${T.line}`, borderRadius: 14, padding: 20 }}>
            <div style={{ fontWeight: 700, fontSize: 16, marginBottom: 8 }}>Leave the game?</div>
            <div style={{ fontFamily: mono, fontSize: 12, color: T.muted, lineHeight: 1.6, marginBottom: 16 }}>This game won't be saved.</div>
            <div style={{ display: "flex", gap: 8 }}>
              <button onClick={() => setConfirmHome(false)} style={{ flex: 1, padding: "11px", borderRadius: 9, border: `1px solid ${T.line}`, cursor: "pointer", background: "rgba(0,0,0,0.3)", color: T.cream, fontFamily: mono, fontSize: 12.5, fontWeight: 700 }}>Stay</button>
              <a href="index.html" style={{ flex: 1, padding: "11px", borderRadius: 9, textAlign: "center", textDecoration: "none", cursor: "pointer", background: `linear-gradient(180deg, ${T.pegRed}, #9c3120)`, color: T.ivory, fontFamily: mono, fontSize: 12.5, fontWeight: 700 }}>Leave</a>
            </div>
          </div>
        </div>
      )}
