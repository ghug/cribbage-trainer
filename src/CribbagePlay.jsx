...
/* ============================ GAME STATE ============================ */
const TARGET = 121;
const PLAYER_OPTIONS = [2, 3, 4]; // table sizes this page supports (heads-up / 3- / 4-handed)
...
const DEFAULT_SETTINGS = { players: 2, counting: "auto", autoGo: false, warn: true, autoDeal: false, autoContinue: false, autoPlayOne: false, autoPlayBest: false, autoDiscardBest: false };
...
function OpponentBacks({ dealerIdx, P, n }) {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
      ...
      {(ts.left != null || ts.right != null) && (
        <div style={{ display: "flex", justifyContent: "space-between", padding: "0 6px" }}>
          {ts.left != null && cell(ts.left)}{ts.right != null && cell(ts.right)}
        </div>
      )}
    </div>
  );
  ...
  return (
    <div style={{ background: "rgba(0,0,0,0.32)", border: `1px solid ${T.line}`, borderRadius: 12, padding: "14px 16px 4px", marginBottom: 14 }}>
      ...
      <Row title="Players at the table" k="players"
        desc="2 = heads-up (deal 6, throw 2). 3 = three-handed cutthroat (deal 5, throw 1; the dealer flips one off the deck to fill the crib). 4 = four-handed cutthroat (deal 5, throw 1). Changing this starts a new game."
