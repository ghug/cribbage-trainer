...
const SEAT_NAMES = ["You", "West", "East"];
...
const initShow = (dealerIdx) => ({
  order: [(dealerIdx + 1) % 3, (dealerIdx + 2) % 3, dealerIdx, "CRIB"],
...
function commitDiscard(state, idx) {
  ...
  // 3-handed: the three discards make only three crib cards, so the dealer fills the
  // fourth straight off the deck (deck[15], the first card after three hands of five).
  const crib = [...seats.map((s) => s.discard), state.deck[15]];
  return { ...state, seats, crib, pendingDiscard: null, phase: "cut" };
}
...
function reduce(state, action) {
  switch (action.type) {
    ...
    case "CUT": {
      const starter = state.deck[16]; // after 3 hands of 5 (15) + the crib filler (deck[15])
    ...
    case "PASS_GO": {
      ...
      if (passes >= 3) { // a full rotation with nobody able to play -> award the go
        let seats = state.seats, message = `${sv(seat, "say", "says")} "go".`;
        const np = { ...peg, passes: 0, turn: (seat + 1) % 3 };
      ...
      return { ...state, peg: { ...peg, passes, turn: (seat + 1) % 3 }, message: `${sv(seat, "say", "says")} "go".` };
    }
    ...
    case "SHOW_SCORE": {
      ...
      if (humanCount) {
        ...
        if (missed > 0 && winner === null) {
          ...
          if (recip === undefined) recip = (state.dealerIdx + 1) % 3; // no opponent left to count -> eldest
    ...
    case "SHOW_NEXT": {
      ...
      if (nextStep >= state.show.order.length)
        return { ...state, phase: "deal", dealerIdx: (state.dealerIdx + 1) % 3, show: null, peg: null, message: "Hand complete — deal the next." };
...
function drawForDealer() {
  for (let attempt = 0; attempt < 500; attempt++) {
    const draw = freshDeck().slice(0, 3); // seat i draws draw[i]
...
function newGameState(prev) {
  ...
  return {
    seats: [0, 1, 2].map((i) => ({ score: 0, isAI: i !== 0, dealt: [], kept: null, discard: null, history: [] })),
...
function ScoreRow({ seats, dealerIdx, turn, winner, onPick }) {
  return (
    <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 6, margin: "0 0 6px" }}>
      {seats.map((s, i) => {
        const isTurn = turn === i;
...
/* ============================ APP ============================ */
export default function CribbagePlay3() {
  ...
  const cutter = (dealerIdx + 2) % 3; // the player to the dealer's right cuts
      ...
      <header style={{
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 12 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 10, minWidth: 0 }}>
            ...
            <span style={{ fontFamily: mono, fontSize: 12, color: "rgba(42,27,14,0.8)", lineHeight: 1.3 }}>3-handed vs 2 bots<br />first to 121</span>
          </div>
        ...
        {phase === "deal" && (
          <div style={{ marginTop: 14, display: "flex", flexDirection: "column", gap: 16 }}>
            <Panel tone={dealer ? "good" : null}>
              ...
              <div style={{ fontFamily: mono, fontSize: 11.5, color: T.muted, marginTop: 3 }}>Each player gets 5 cards and throws one to the crib; the dealer adds one off the deck to make four. First to 121 wins.</div>
            </Panel>
        ...
        {phase === "cut" && (
          <div style={{ marginTop: 14, display: "flex", flexDirection: "column", gap: 16 }}>
            <Panel>
              ...
              <div style={{ fontFamily: mono, fontSize: 11.5, color: T.muted, marginTop: 3 }}>
                Four cards in {dealer ? "your" : `${seatName(dealerIdx)}'s`} crib — three throws plus one off the deck.
              </div>
            </Panel>
...
// West (1) and East (2) flank the table — your two opponents, one to each side.
...
function PlayScreen({ state, dispatch }) {
  ...
  return (
    <div style={{ marginTop: 12, display: "flex", flexDirection: "column", gap: 14 }}>
      {/* the table: West — STARTER — East across the middle (your two opponents) */}
