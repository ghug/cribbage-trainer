...
const SEAT_NAMES = ["You", "West", "Northwest", "Northeast", "East"];
...
const initShow = (dealerIdx) => ({
  order: [(dealerIdx + 1) % 5, (dealerIdx + 2) % 5, (dealerIdx + 3) % 5, (dealerIdx + 4) % 5, dealerIdx, "CRIB"],
...
function dealNewHand(state) {
  ...
  // The dealer is dealt four and keeps them all; everyone else is dealt five and
  // throws one. Hand sizes vary by seat, so slice the deck sequentially.
  let off = 0;
  ...
  // The dealer makes no throw (keeps four); every non-dealer bot throws now. The human
  // (seat 0), when not the dealer, throws during the discard phase.
  for (let i = 0; i < 5; i++) {
    if (i === d) { seats[i].kept = sortHand(seats[i].dealt); }
  ...
  // When the human deals, they make no throw — the crib is already the four non-dealer
  // throws — so skip the discard phase and go straight to the cut.
  if (d === 0) return { ...base, crib: assembleCrib(seats), phase: "cut", message: "Your deal — four cards, no throw. The crib is yours." };
  return base;
}
...
function reduce(state, action) {
  switch (action.type) {
    ...
    case "CUT": {
      const starter = state.deck[24]; // after the deal: dealer 4 + four hands of 5 = 24
    ...
    case "PASS_GO": {
      ...
      if (passes >= 5) { // a full rotation with nobody able to play -> award the go
        let seats = state.seats, message = `${sv(seat, "say", "says")} "go".`;
        const np = { ...peg, passes: 0, turn: (seat + 1) % 5 };
      ...
      return { ...state, peg: { ...peg, passes, turn: (seat + 1) % 5 }, message: `${sv(seat, "say", "says")} "go".` };
    }
    ...
    case "SHOW_SCORE": {
      ...
      if (humanCount) {
        ...
        if (missed > 0 && winner === null) {
          ...
          if (recip === undefined) recip = (state.dealerIdx + 1) % 5; // no opponent left to count -> eldest
    ...
    case "SHOW_NEXT": {
      ...
      if (nextStep >= state.show.order.length)
        return { ...state, phase: "deal", dealerIdx: (state.dealerIdx + 1) % 5, show: null, peg: null, message: "Hand complete — deal the next." };
...
function drawForDealer() {
  for (let attempt = 0; attempt < 500; attempt++) {
    const draw = freshDeck().slice(0, 5); // seat i draws draw[i]
...
function newGameState(prev) {
  ...
  return {
    seats: [0, 1, 2, 3, 4].map((i) => ({ score: 0, isAI: i !== 0, dealt: [], kept: null, discard: null, history: [] })),
...
function ScoreRow({ seats, dealerIdx, turn, winner, onPick }) {
  return (
    <div style={{ display: "grid", gridTemplateColumns: "repeat(5,1fr)", gap: 6, margin: "0 0 6px" }}>
      {seats.map((s, i) => {
        const isTurn = turn === i;
...
/* ============================ APP ============================ */
export default function CribbagePlay5() {
  ...
  const cutter = (dealerIdx + 4) % 5; // the player to the dealer's right cuts
      ...
      <header style={{
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 12 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 10, minWidth: 0 }}>
            ...
            <span style={{ fontFamily: mono, fontSize: 12, color: "rgba(42,27,14,0.8)", lineHeight: 1.3 }}>5-handed vs 4 bots<br />first to 121</span>
          </div>
        ...
        {phase === "deal" && (
          <div style={{ marginTop: 14, display: "flex", flexDirection: "column", gap: 16 }}>
            <Panel tone={dealer ? "good" : null}>
              ...
              <div style={{ fontFamily: mono, fontSize: 11.5, color: T.muted, marginTop: 3 }}>Everyone is dealt 5 and throws one to the crib — except the dealer, dealt 4 and keeping them all. First to 121 wins.</div>
            </Panel>
...
// Northwest (2) and Northeast (3) across the top; West (1) and East (4) flank below.
function OpponentRows({ render }) {
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
      <div style={{ display: "flex", justifyContent: "space-around" }}>{render(2)}{render(3)}</div>
...
function PlayScreen({ state, dispatch }) {
  ...
  return (
    <div style={{ marginTop: 12, display: "flex", flexDirection: "column", gap: 14 }}>
      {/* the table: Northwest & Northeast on top; West — STARTER — East across the middle */}
      <div style={{ display: "flex", justifyContent: "space-around" }}>{cell(2)}{cell(3)}</div>
