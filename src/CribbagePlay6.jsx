...
const SEAT_NAMES = ["You", "West", "Northwest", "North", "Northeast", "East"];
...
const initShow = (dealerIdx) => ({
  order: [(dealerIdx + 1) % 6, (dealerIdx + 2) % 6, (dealerIdx + 3) % 6, (dealerIdx + 4) % 6, (dealerIdx + 5) % 6, dealerIdx, "CRIB"],
...
// The 4-card crib is the throws from every non-dealer seat (the dealer makes none).
function assembleCrib(seats) {
  return seats.filter((s) => s.discard).map((s) => s.discard);
}

// Commit a chosen throw: assemble the crib from the non-dealer throws and cut next.
function commitDiscard(state, idx) {
  ...
  return { ...state, seats, crib: assembleCrib(seats), pendingDiscard: null, phase: "cut" };
}

function dealNewHand(state) {
  const deck = freshDeck();
  const d = state.dealerIdx;
  const right = (d + 5) % 6;            // the player to the dealer's right
  // The dealer and the player to their right are dealt four and keep them all; the
  // other four are dealt five and throw one. Slice sequentially since sizes vary.
  const noThrow = (i) => i === d || i === right;
  let off = 0;
  const seats = [0, 1, 2, 3, 4, 5].map((i) => {
    const size = noThrow(i) ? 4 : 5;
    const dealt = sortHand(deck.slice(off, off + size));
    off += size;
    return { score: state.seats[i].score, isAI: i !== 0, history: state.seats[i].history || [], dealt, kept: null, discard: null };
  });
  // Non-throwers keep their four; every throwing bot throws now. The human (seat 0),
  // when a thrower, throws during the discard phase.
  for (let i = 0; i < 6; i++) {
    if (noThrow(i)) { seats[i].kept = sortHand(seats[i].dealt); }
    else if (i !== 0) { const { discard, kept } = aiDiscard(seats[i].dealt, i, d); seats[i].discard = discard; seats[i].kept = sortHand(kept); }
  }
  const base = {
  ...
  // When the human makes no throw (they deal, or sit to the dealer's right), the crib
  // is already the four throwers' cards — skip the discard phase, go to the cut.
  if (noThrow(0)) {
    const msg = d === 0 ? "Your deal — four cards, no throw. The crib is yours."
      : "You're to the dealer's right — four cards, no throw this hand.";
    return { ...base, crib: assembleCrib(seats), phase: "cut", message: msg };
  }
  return base;
}
...
function reduce(state, action) {
  switch (action.type) {
    ...
    case "CUT": {
      const starter = state.deck[28]; // after the deal: two 4-card hands + four of 5 = 28
    ...
    case "PASS_GO": {
      ...
      if (passes >= 6) { // a full rotation with nobody able to play -> award the go
        let seats = state.seats, message = `${sv(seat, "say", "says")} "go".`;
        const np = { ...peg, passes: 0, turn: (seat + 1) % 6 };
      ...
      return { ...state, peg: { ...peg, passes, turn: (seat + 1) % 6 }, message: `${sv(seat, "say", "says")} "go".` };
    }
    ...
    case "SHOW_SCORE": {
      ...
      if (humanCount) {
        ...
        if (missed > 0 && winner === null) {
          ...
          if (recip === undefined) recip = (state.dealerIdx + 1) % 6; // no opponent left to count -> eldest
    ...
    case "SHOW_NEXT": {
      ...
      if (nextStep >= state.show.order.length)
        return { ...state, phase: "deal", dealerIdx: (state.dealerIdx + 1) % 6, show: null, peg: null, message: "Hand complete — deal the next." };
...
function drawForDealer() {
  for (let attempt = 0; attempt < 500; attempt++) {
    const draw = freshDeck().slice(0, 6); // seat i draws draw[i]
...
function newGameState(prev) {
  ...
  return {
    seats: [0, 1, 2, 3, 4, 5].map((i) => ({ score: 0, isAI: i !== 0, dealt: [], kept: null, discard: null, history: [] })),
...
function ScoreRow({ seats, dealerIdx, turn, winner, onPick }) {
  return (
    <div style={{ display: "grid", gridTemplateColumns: "repeat(6,1fr)", gap: 6, margin: "0 0 6px" }}>
      {seats.map((s, i) => {
        const isTurn = turn === i;
...
/* ============================ APP ============================ */
export default function CribbagePlay6() {
  ...
  const cutter = (dealerIdx + 5) % 6; // the player to the dealer's right cuts
      ...
      <header style={{
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 12 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 10, minWidth: 0 }}>
            ...
            <span style={{ fontFamily: mono, fontSize: 12, color: "rgba(42,27,14,0.8)", lineHeight: 1.3 }}>6-handed vs 5 bots<br />first to 121</span>
          </div>
        ...
        {phase === "deal" && (
          <div style={{ marginTop: 14, display: "flex", flexDirection: "column", gap: 16 }}>
            <Panel tone={dealer ? "good" : null}>
              ...
              <div style={{ fontFamily: mono, fontSize: 11.5, color: T.muted, marginTop: 3 }}>Everyone is dealt 5 and throws one — except the dealer and the player to their right, dealt 4 and keeping them all. First to 121 wins.</div>
            </Panel>
...
// Northwest (2), North (3), Northeast (4) across the top; West (1) and East (5) flank.
function OpponentRows({ render }) {
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
      <div style={{ display: "flex", justifyContent: "space-around" }}>{render(2)}{render(3)}{render(4)}</div>
...
function PlayScreen({ state, dispatch }) {
  ...
  return (
    <div style={{ marginTop: 12, display: "flex", flexDirection: "column", gap: 14 }}>
      {/* the table: NW, N, NE across the top; West — STARTER — East across the middle */}
      <div style={{ display: "flex", justifyContent: "space-around" }}>{cell(2)}{cell(3)}{cell(4)}</div>
